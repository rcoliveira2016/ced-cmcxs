...
//
  ...
  public function validarHoraBanco($f, $i, $e, $d){
    ...
    $sql='SELECT *
        ...
        (TIME_TO_SEC(f_hora) >= TIME_TO_SEC("'.$f.'") and TIME_TO_SEC(i_hora) <= TIME_TO_SEC("'.$f.'"))) and id_espaco="'.$e.'" and data="'.$data.'"';
  ...
  public function addSolicitantes($arr){
    ...
    $sql="INSERT INTO `solicitacoes`(`id_espaco`, `id_solicitante`, `assunto`, `id_categoria`, `data`, `i_hora`, `f_hora`, `desc`, `id_user`, site_camara) VALUES
     ({$arr['espaco']}, {$arr['solicitantes']}, '{$arr['assunto']}', {$arr['categoria']}, '$data', '{$arr['h_inicial']}', '{$arr['h_final']}', '$d', {$arr['id']}, $interno)";
  ...
  public function getRelCatSocData($id, $data, $s){
    ...
    $sql="SELECT *,DATE_FORMAT(data, '%d/%m/%Y') as dataF FROM `solicitacoes`,cadas_categoria WHERE cadas_categoria.id_categoria=solicitacoes.id_categoria and cadas_categoria.id_categoria=$id and DATE_FORMAT(data, '%d/%m/%Y')='$data' $where ";
    $rs=mysql_query($sql);
    $a=array();
    while ($registro = mysql_fetch_array($rs)) {
        $id = $registro[0];
